...
#define AMBIENT_LIGHT           10			// Darkest shade for object colors (0-63)
#define SPECULARITY             63			// Brightest shade for object colors (0-63)
...
#define REVOLUTIONS_PER_MINUTE  4
...
//
//   Object color gradients follow a two-phase path from ambient color
//   to white, passing through the full-brightness base color. The
//   SPECULARITY constant controls how far along this path each gradient
//   extends (0 = flat ambient, 63 = full range to white).
//
// Arguments:
//
//   - None
...
void InitializePalette ( void )
{
	...
	int ambient_light = AMBIENT_LIGHT;
	int specularity   = SPECULARITY;
	...
	// inactive (0). The gradient follows a two-phase path controlled
	// by specularity:
	//
	//   Phase 1 (diffuse):  Active channels ramp from ambient_light
	//                       to 63. Inactive channels stay at 0.
	//   Phase 2 (specular): Active channels hold at 63. Inactive
	//                       channels ramp from 0 toward 63 (white).
	//
	// Specularity controls how far along this path the gradient
	// extends: 0 = flat ambient color, 63 = full range to white.
	//
	//   Color     | R | G | B
	//   ----------+---+---+---
	//   Greyscale | 1 | 1 | 1
	...
	// Total path distance from ambient color to white

	int diffuse_range  = 63 - ambient_light;
	int total_distance = diffuse_range + 63;

	for ( int color = 0; color < 7; color++ )
	{
		for ( shade = 0; shade < SHADE_COUNT; shade++ )
		{
			// Position along the ambient-to-white path, scaled by specularity.
			// Uses long arithmetic to avoid 16-bit overflow (max product: 226,548).

			long position = ( (long) shade * specularity * total_distance ) / ( (long) ( SHADE_COUNT - 1 ) * 63 );

			int active_value;
			int inactive_value;

			if ( position <= diffuse_range )
			{
				// Phase 1: Brightening the base color

				active_value   = ambient_light + (int) position;
				inactive_value = 0;
			}
			else
			{
				// Phase 2: Washing toward white

				active_value   = 63;
				inactive_value = (int) ( position - diffuse_range );
			}

			palette [ index++ ] = red_channel   [ color ] ? active_value : inactive_value;
			palette [ index++ ] = green_channel [ color ] ? active_value : inactive_value;
			palette [ index++ ] = blue_channel  [ color ] ? active_value : inactive_value;
		}
	}
...
int main ( void )
{
	...
	CALC yaw_rate   = ( 3.0 * REVOLUTIONS_PER_MINUTE * 2.0 * M_PI ) / 60.0;
	CALC pitch_rate = (       REVOLUTIONS_PER_MINUTE * 2.0 * M_PI ) / 60.0;
